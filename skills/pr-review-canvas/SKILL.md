...
4. Start a local server on a fixed port:
   ...
   ```
   Run this backgrounded, then open `http://127.0.0.1:8432/pr-review-{number}.html` in a browser.
...
> **Attribution:** Ported from [`cursor-team-kit/skills/pr-review-canvas`](https://github.com/cursor/plugins/blob/3347cbab5b54136f6fba0994c3a01a56f7fb7fca/cursor-team-kit/skills/pr-review-canvas/SKILL.md) by Team Cursor, MIT-licensed. Adapted to Claude Code primitives by bdigital media — frontmatter `disable-model-invocation: true` → `user-invocable: true`, and the "in-app browser" reference in step 4 generalized to "a browser" since Claude Code parents don't expose Cursor's in-app browser surface. The `renderer.js`, `styles.css`, and `template.html` assets in this directory are preserved verbatim from upstream. See [`PORTING-NOTES.md`](../../PORTING-NOTES.md).
